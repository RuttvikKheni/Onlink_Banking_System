...
require_once "session.php";
    ...
    function login_attempt($logid,$password){
        global $con;
        $sql = "SELECT * from employees_master WHERE loginid=:logid and (employee_type='Staff' or employee_type='Manager') AND pwd=:passWord LIMIT 1";
        $stmt = $con->prepare($sql);
        $stmt->bindValue(':logid',$logid);
    ...
    function Total_Customer() {
        ...
        $sql = "SELECT COUNT(*) FROM customers_master WHERE ifsccode='$ifsccode'";
        $stmt = $con->query($sql);
    ...
    function total_admin(){
        global $con;
        $id = $_SESSION['id'];
        $sql = "SELECT COUNT(*) FROM employees_master WHERE id='$id'";
        $stmt = $con->query($sql);
        $totalRows = $stmt->fetch();
        $total = array_shift($totalRows);
        echo $total;
    ...
    function total_credit_account(){
        ...
        $sql = "SELECT * FROM accounts INNER JOIN customers_master ON customers_master.c_id=accounts.c_id WHERE customers_master.ifsccode='$ifsccode'";
        $stmt = $con->query($sql);
        while ($row = $stmt->fetch()){
            $c_id = $row['c_id'];
        }
    ...
    function total_accounts() {
        global $con;
        $id = $_SESSION['id'];
        $sql = "SELECT * FROM employees_master WHERE id='$id'";
        $stmt = $con->query($sql);
        while ($row = $stmt->fetch()) {
            $ifsccode = $row['ifsccode'];
        }
//        Saving Account
        $sql = "SELECT COUNT(accounts.account_type) FROM customers_master INNER JOIN accounts ON customers_master.c_id=accounts.c_id WHERE customers_master.ifsccode='$ifsccode' AND accounts.account_type='Saving Account'";
        $stmt = $con->query($sql);
        $totalRows = $stmt->fetch();
        $totalSaving_acc = array_shift($totalRows);
//      Current Account
        $sql = "SELECT COUNT(accounts.account_type) FROM customers_master INNER JOIN accounts ON customers_master.c_id=accounts.c_id WHERE customers_master.ifsccode='$ifsccode' AND accounts.account_type='Current Account'";
        $stmt = $con->query($sql);
        $totalRows = $stmt->fetch();
        $totalCurrent_acc = array_shift($totalRows);
//        Fixed Deposite Account
        $sql = "SELECT COUNT(accounts.account_type) FROM customers_master INNER JOIN accounts ON customers_master.c_id=accounts.c_id WHERE customers_master.ifsccode='$ifsccode' AND accounts.account_type='Fixed Deposite Account'";
        $stmt = $con->query($sql);
        $totalRows = $stmt->fetch();
