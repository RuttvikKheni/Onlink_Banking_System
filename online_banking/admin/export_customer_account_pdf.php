...
//    private $_password;
    ...
    public function ColoredTable($header,$data) {
        ...
        $this->SetFont('', 'B');
        // Header
        $w = array(8,25,25,25,65,40,40,40,50,40,30,30,30);
        ...
        $fill = 0;
        foreach($data as $row) {
            $this->Cell($w[0], 6, $row['c_id'], 0, 'L', $fill);
            $this->Cell($w[1], 6, $row['ifsccode'], 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row['f_name'], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row['l_name'], 'LR', 0, 'R', $fill);
            $this->Cell($w[4], 6, $row['email'], 'LR', 0, 'R', $fill);
            $this->Cell($w[5], 6, $row['phone'], 'LR', 0, 'R', $fill);
            $this->Cell($w[6], 6, $row['city'], 'LR', 0, 'R', $fill);
            $this->Cell($w[7], 6, $row['adharnumber'], 'LR', 0, 'R', $fill);
            $this->Cell($w[8], 6, $row['gender'], 'LR', 0, 'R', $fill);
            $this->Cell($w[9], 6, $row['occuption'], 'LR', 0, 'R', $fill);
            $this->Cell($w[10], 6, $row['accountstatus'], 'LR', 0, 'R', $fill);
            $this->Ln();
            $fill=!$fill;
        }
...
// column titles
$header = array('ID','Branch','First Name','Last Name','Email','Mobile Number','city','Adhar Number','Gender','Occupation','Status');
