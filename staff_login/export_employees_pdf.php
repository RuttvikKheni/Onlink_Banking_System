...
//    private $_password;
    ...
    public function ColoredTable($header,$data) {
        ...
        foreach($data as $row) {
            ...
            $this->Cell($w[1], 6, $row['ename'], 0, 'L', $fill);
            $this->Cell($w[2], 6, $row['loginid'], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row['email'], 'LR', 0, 'R', $fill);
            $this->Cell($w[4], 6, $row['contact'], 'LR', 0, 'R', $fill);
            $this->Cell($w[5], 6, $row['employee_type'], 'LR', 0, 'R', $fill);
...
// column titles
$header = array('IFSC Code','Employee Name','Login ID','Email ID','Contact','Positions','Status');
